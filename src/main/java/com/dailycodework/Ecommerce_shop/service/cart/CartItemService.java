...
public class CartItemService implements ICartItemService{
    ...
    public void removeItemFromCart(Long cartId, Long productId) {
        ...
        CartItem itemToRemove = cart.getItems()
                .stream()
                .filter(item -> item.getProduct().getId().equals(productId))
                .findFirst().orElseThrow(() -> new ResourceNotFoundException("Product Not Found Here!"));
...
//        Cart cart=iCartService.getCart(cartId);
//        cart.getItems().stream().filter(item-> item.getId().equals(productId))
//                .findAny()
//                .ifPresent(item->{
...
//        cartRepository.save(cart);
        ...
        cart.getItems().stream()
                ...
                .ifPresent(item -> {
                    ...
                    item.setUnitPrice(item.getProduct().getPrice());
                    item.setTotalPrice();
                    System.out.println("Updated CartItem: " + item);
                });

//        ca(); // Ensure total amount is updated
